...
const ADMIN_UID = 'Lu389LSbC7dVbAfm5DFF8w23b5I2';
...
export default async function handler(req, res) {
  ...
  try {
    await firestore.collection('reviews').doc(id).update({ confirmed });
    res.status(200).json({ success: true });
  } catch (e) {
    res.status(500).json({ error: '업데이트 실패', detail: e.message });
  }
}
